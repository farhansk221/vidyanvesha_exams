...
export const ExamQuestionOutcomeService = {
    async getAll(): Promise<PaginatedResponse<ExamQuestionOutcome>> {
        const response = await api.get<PaginatedResponse<ExamQuestionOutcome>>(API_CONTS.EXAM_QUESTION_OUTCOMES.LIST);
    ...
    async getById(id: number): Promise<ExamQuestionOutcome> {
        ...
        const response = await api.get<ExamQuestionOutcome>(url);
        return response.data;
    },

    async create(data: Omit<ExamQuestionOutcome, "id">): Promise<ExamQuestionOutcome> {
        const response = await api.post<ExamQuestionOutcome>(API_CONTS.EXAM_QUESTION_OUTCOMES.CREATE, data);
        return response.data;
    },
    ...
    async update(id: number, data: Omit<ExamQuestionOutcome, "id">): Promise<ExamQuestionOutcome> {
        ...
        const response = await api.put<ExamQuestionOutcome>(url, data);
        return response.data;
    },
